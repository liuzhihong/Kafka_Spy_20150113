...
import com.suning.app.spy.core.answer.AnswerControl;
...
public class Agent {
	...
	private static final Configuration commonConf = new Configuration(Constants.COMMON_CONF_FILE);
	...
	private TmpMetricsModel tmpMetricsModel;
	...
	public void sendResult(MetricsInfoModel resultInfo) {
		...
		try {
			...
			tmpMetricsModel.setMetricsModel(resultInfo);
		} finally {
			lock.unlock();
		}
	...
	public void doSend(TmpMetricsModel model) {
	...
	public void init() {
		...
		tmpMetricsModel = new TmpMetricsModel(metricsTarget);
		
		AnswerControl.getInstance().start(commonConf);
		if (!AnswerControl.getInstance().isStarting()) {
			LOGGER.error("answerControl start eror!");
			System.exit(-1);
		}
		
		AnswerControl.getInstance().init(metricsTarget);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				while (true) {
					try {
						lock.lock();
						if (null != tmpMetricsModel && tmpMetricsModel.size() > 0) {
							AnswerControl.getInstance().send(tmpMetricsModel);
						}
					} finally {
						tmpMetricsModel.clear();
						lock.unlock();
					}
