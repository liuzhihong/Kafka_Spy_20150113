...
import com.suning.shared.spy.model.biz.DiskCatalogInfoModel;
import com.suning.shared.spy.model.biz.DiskIOInfoModel;
...
public class DiskInfoComandImpl extends AbstractCommandHandler<DiskInfoModel> {
    ...
    public DiskInfoModel doHandle(String[] cmd) {
        ...
        Map<String,DiskIOInfoModel> ioInfo = new HashMap<String,DiskIOInfoModel>();
            ...
            for(String result : comResults){
                if(++count >= 4){
                    ...
                    if(data.length > 1 && data[data.length-1].matches("[0-9]+\\.[0-9]+")){
                        DiskIOInfoModel ioInfoModel = new DiskIOInfoModel();
                        ioInfoModel.setReadSpeed(Float.parseFloat(data[5]));
                        ioInfoModel.setWriteSpeed(Float.parseFloat(data[6]));
                        ioInfoModel.setAwaitTime(Float.parseFloat(data[data.length-3]));
                        ioInfoModel.setPercentUtil(Float.parseFloat(data[data.length-1]));
                        ioInfo.put(data[0], ioInfoModel);
                    }
