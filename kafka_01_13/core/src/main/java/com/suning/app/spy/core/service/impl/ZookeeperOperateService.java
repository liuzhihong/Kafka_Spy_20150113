...
public class ZookeeperOperateService extends AbstractLifeCycle implements Service {
	...
	public void Send(TmpMetricsModel model) {
		if (null == model) {
			return;
		}
		...
		try {
			zkClient.writeData(path,json.getBytes());
		} catch(ZkNoNodeException e) {
			LOGGER.warn("reCreate node!", e);
			creatNode(model.getMetricsTarget());
		}catch (Exception e) {
			LOGGER.warn("zk send data occured some error!", e);
		}
	}
