...
public class SpyClient {
	...
	private static final Logger LOGGER = Logger.getLogger(SpyClient.class);

	private static SpyClient ins = new SpyClient();

	private SpyClient() {

	}

	public static SpyClient getInstance() {
		return ins;
	}
		...
		for (MetricsTarget metricsTarget : metricsTargetList) {
			...
			zkClient.subscribeDataChanges(node, new IZkDataListener() {
				...
				public void handleDataChange(String path, byte[] data) throws Exception {
					...
					TmpMetricsModel uModel = null;
					try{
						uModel = JSON.parseObject(dataStr, TmpMetricsModel.class);
//						uModel = UnitedMetricsModel.deserialize(data);
					} catch (Exception e) {
						LOGGER.error("Metrics json data parse error: "+dataStr, e);
					}
					UnitedMetricsModel model = new UnitedMetricsModel(uModel.getMetricsModels(), uModel.getMetricsTarget());
					
					if (valid(path, model)) {
						if (null != onDataChangeListenerInvoker) {
							onDataChangeListenerInvoker.invoke(model);
						}
					}
				}
	...
	public class SpyAttachHolder {
		...
		public SpyListenerHolder attach(List<MetricsTarget> metricsTargetList) {
			...
			SpyClient.this.metricsTargetList = metricsTargetList;
			return new SpyListenerHolder();
		}

		public void start() {

			SpyClient.this.start();
		}
	...
	public class SpyListenerHolder {
		...
		public SpyListenerHolder onConnect(StateListenerInvoker listenerInvoker) {
			SpyClient.this.onConnectListenerInvoker = listenerInvoker;
			return this;
		}

		public SpyListenerHolder onDataChange(DataListenerInvoker listenerInvoker) {
			SpyClient.this.onDataChangeListenerInvoker = listenerInvoker;
			return this;
		}

		public SpyListenerHolder onDisConnect(StateListenerInvoker listenerInvoker) {
			SpyClient.this.onDisConnectListenerInvoker = listenerInvoker;
			return this;
		}
